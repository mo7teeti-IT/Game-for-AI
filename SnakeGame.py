...
def message(msg, color):
    ...
    dis.blit(mesg, [(dis_width - mesg_width) / 2, (dis_height - mesg_height) / 2])
...
def gameLoop():  # Creating a function for the game loop
    ...
    while not game_over:
        ...
        while game_close:
            dis.fill(blue)
            message("You Lost! Press C to Play Again or Q to Quit", red)  # Center the message
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_a:  # 'A' for left
                    x1_change = -snake_block
                    y1_change = 0
                elif event.key == pygame.K_d:  # 'D' for right
                    x1_change = snake_block
                    y1_change = 0
                elif event.key == pygame.K_w:  # 'W' for up
